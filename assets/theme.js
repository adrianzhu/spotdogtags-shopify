...
window.AjaxCart = (function() {
  ...
  cart.prototype.addItemFromForm = function(evt) {
    ...
    var hasError = false;
    this.$form.find('input.tag-customizer-input').each(function() {
      // if there are any special characters then show error
      // http://facweb.cs.depaul.edu/sjost/it212/documents/ascii-pr.htm
      // regex matches anything that is not between ' ' and '~' ASCII values
      var regex = /[^ -~]/; 
      if (regex.test($(this).val())) {
        theme.Notify.open('error', 'Some characters entered are not allowed. Please remove all special characters to continue.', true);
        hasError = true;
    ...
    this.$form.find('input.tag-customizer-input').each(function() {
      // If field is empty, replace with special empty char to show it in cart/checkout
      // This char needs to be removed from backend at time of image generation
      if ($(this).val() == false) {
        $(this).val('\u200F'); 
      }
